...
import articlesdata.sentence.gene.DistributedMentionRecognizer;
import articlesdata.sentence.neghedge.SentenceEntry;

/**
 * @author qing
 *
 */
public class DistributedDBDrivenCitationNetworkBuilder extends Configured
		implements Tool {

	private static Logger LOGGER = NetworkBuilderLogger.getLogger("DistributedDBDrivenCitationNetworkBuilder");
	...
	public int run(String[] args) throws Exception {
        ...
        job.setJobName("distributed-db-driven-citation-network-builder");
...
//        		" AND (citation_detail.citation_id NOT in (select citation_id from cnetworkv4 where +"+DataDrivenDBInputFormat.SUBSTITUTE_TOKEN +"))";
        ...
        job.setMapperClass(NetworkDBDrivenBuilderMap.class);
	...
	public static void main(String[] args) {
		try {
            int ret = ToolRunner.run(new DistributedDBDrivenCitationNetworkBuilder(), args);
            System.exit(ret);
        } catch (Exception ex) {
